...
inline varset nextPermutation( varset &vs ){ // TODO: Need optmization
	std::vector<bool> v( vs.size() , false ) ;
	for(int i = 0 ; i < vs.size() ; i++)
		v[ vs.size() - i - 1 ] = VARSET_GET( vs , i ) ;
	std::next_permutation( v.begin() , v.end() ) ;
	VARSET_NEW( perm , vs.size() ) ;
	VARSET_CLEAR_ALL( perm ) ;
	for(int i = 0; i < perm.size() ; i++)
		if( v[ i ] )
			VARSET_SET( perm , vs.size() - i - 1 ) ;
	return perm ;
}
...
inline varset stringToVarset(const std::string &str) {
    varset vs;
    vs = boost::lexical_cast<uint64_t>(str);
    return vs;
}
